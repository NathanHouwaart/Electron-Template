...
static void DeriveSessionKey(const byte rnda[8], const byte rndb[8], byte outSessionKey[16], byte outSessionKeyTrace[16])
{
    // Build session_key_calc: rnda[0..3], rndb[0..3], rnda[4..7], rndb[4..7]
    for (int i = 0; i < 4; ++i) outSessionKey[i] = rnda[i];
    for (int i = 0; i < 4; ++i) outSessionKey[4 + i] = rndb[i];
    for (int i = 0; i < 4; ++i) outSessionKey[8 + i] = rnda[4 + i];
    for (int i = 0; i < 4; ++i) outSessionKey[12 + i] = rndb[4 + i];

    // Build session_key_trace: mask LSB (parity) off for first 8 bytes and duplicate
    for (int i = 0; i < 8; ++i)
    {
        outSessionKeyTrace[i] = outSessionKey[i] & 0xFE;
    }
    // duplicate the first 8 masked bytes
    for (int i = 0; i < 8; ++i)
    {
...
int _tmain(int argc, _TCHAR* argv[])
{
    ...
    }
    {
        ...
        DeriveSessionKey(RndA, RndB, sessionKey, sessionKeyTrace);
        ...
        Utils::Print("* Session Key:        "); Utils::PrintHexBuf(sessionKey, 16, LF);
        Utils::Print("* Session Key Trace:  "); Utils::PrintHexBuf(sessionKeyTrace, 16, LF);
        ...
        for (int i = 0; i < 16; i++) cryptogram.push_back(newKey[i]);
        for (int i = 0; i < 4; i++) cryptogram.push_back((u32_Crc_crypto >> (8 * i)) & 0xFF);
        for (int i = 0; i < 4; i++) cryptogram.push_back(0x00);
        Utils::Print("\*** Cryptogram [New Key, CRC Crypto, CRC NewKey]:", LF);
        Utils::PrintHexBuf((const byte*)cryptogram.data(), (int)cryptogram.size(), LF);
...

    {
        ...
        byte currentKey[16] = { 0x00, 0x10, 0x20, 0x31, 0x40, 0x50, 0x60, 0x70, 0x80, 0x90, 0xA0, 0xB0, 0xB0, 0xA0, 0x90, 0x80 };
